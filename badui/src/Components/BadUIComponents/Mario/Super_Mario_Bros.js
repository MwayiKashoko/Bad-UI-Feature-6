...
export const gameEngine = (canvas) => {
    ...
    const keyHandlers = {
        ...
        KeyX(e) {
            if (e.repeat || mario.clearedLevel) return;

            if (
                mario.hasFireFlower &&
                (fireballsThrown % 2 === 1 || timeUntilNextFireball <= 0) &&
                !mario.isCrouching
            ) {
                sounds[4].currentTime = 0;
                sounds[4].play();
                ...
                fireballs.push(
                    new Projectile(
                        "Fireball",
                        mario.directionFacing === "left"
                            ? mario.drawnX
                            : mario.drawnX + mario.width / 2,
                        mario.drawnY,
    ...
    document.addEventListener("keydown", e => {
        if (quit || state !== "game" || mario.transition) return;
    ...
    const setBackground = () => {
        ...
        if (currentLocation.terrain === "Underwater") {
            // Create one Image per frame, not per tile
            const water = new Image();
            water.src = `${pathname}/images/WaterTop.png`;
            const y = standardHeight * 2;

            for (let i = 0; i < 17; i++) {
                graphics.drawImage(water, i * standardWidth, y, standardWidth, standardHeight);
            }
        ...
        if (bg) {
            if (canUpdate) {
                const scroll = currentLocation.canScroll && mario.drawnX >= width / 2 - shiftWidth;
                bg.update(movingScreen, scroll, mario.velX);
            }
            bg.draw();
        }
    };
    ...
    const setCoins = () => {
        const coinList = mario.coinAnimationList;
        const frame = Math.floor(time / 4 % 4) + 1; // 1–4

        // iterate backwards for safe removal
        for (let i = coinList.length - 1; i >= 0; i--) {
            const coin = coinList[i];

            // only create the image once per coin
            if (!coin.img) {
                coin.img = new Image();
            }

            // update the frame
            const src = `${pathname}/images/CoinSpin${frame}.png`;
            if (coin.img.src !== src) {
                coin.img.src = src;
            }
    ...
    const setScores = () => {
        const scoreValues = mario.scoreValues;
        ...
        for (let i = scoreValues.length - 1; i >= 0; i--) {
            ...
            if (score.timer > 50) {
                scoreValues.splice(i, 1);
                continue; // skip drawing removed scores
            }

            // Draw score
            if (score.score !== 50) {
                graphics.fillText(score.score, score.x + 18, score.y);
            }
        }
    };
    ...
    const reset = () => {
        randomized = false;
        state = "loading";
        timeUntilPlay = mario.lives === 0 ? 350 : 150;
        ...
        const areasArray = levels[level].areas;
        for (let k = 0; k < areasArray.length; k++) {
            const areas = areasArray[k];
            areas.pair = null;

            // Reset blocks
            for (let j = 0; j < areas.area.length; j++) {
                const row = areas.area[j];
                for (let i = 0; i < row.length; i++) {
                    const block = row[i];
                    const canEnter = block.canEnter;
                    const isEdge = block.isEdge;
                    const pair = block.pair;
                    const pulleyPair = block.pulleyPair;
                    ...
                    row[i] = new Block(
                        ...
                        block.constantY,
                        block.constantWidth,
                        block.constantHeight,
                        sounds,
                        block.terrain
                    );

                    row[i].canEnter = canEnter;
                    row[i].isEdge = isEdge;
                    row[i].pair = pair;
                    row[i].pulleyPair = pulleyPair;
                    if (pulleyPair != null) pulleyPair.pulleyPair = row[i];
                    row[i].connection = connection;
                }
            }

            // Reset background
            if (areas.background != null) {
                const bg = areas.background;
                areas.background = new Background(bg.img.src, bg.constantX, bg.constantY, bg.width, bg.height);
            }

            // Reset enemies
            if (areas.enemies != null) {
                let canResetEnemy = true;
                const enemies = areas.enemies;
                for (let i = 0; i < enemies.length; i++) {
                    const enemy = enemies[i];
                    if (canResetEnemy) {
                        enemies[i] = new Enemy(enemy.constantX, enemy.constantY, enemy.constantWidth, enemy.constantHeight, enemy.storedType, gravity, sounds, areas.terrain, enemy.animated);
                    }

                    if (enemies[i].drawnX === -1000) canResetEnemy = false;
                }

                // Trim enemies array
                for (let i = 0; i < enemies.length; i++) {
                    if (enemies[i].drawnX === -1000) {
                        enemies.length = i + 1;
                        break;
                    }
                }
        ...
        // Recreate player
        const prevCoins = mario.coins;
        const prevLives = mario.lives;
        mario = new Player(prevLives, gravity, music, sounds, prevCoins);

        mario.drawnY = mario.isBig ? 440 : 480;
        if (stage === 4) {
            mario.drawnY = 240 - (mario.height - 40);
        }
    ...
    const nextLevel = () => {
        ...
        // Reset player horizontal and other states
        mario.drawnX = 80;
        mario.alignX = 80;
        mario.velX = 0;
        mario.lastGroundY = mario.drawnY;
        ...
        // Ensure current location is set correctly
        currentLocation = levels[level].areas[0];
    };
    ...
    const changeLocation = () => {
        ...
        // Set paired location if Mario is standing on a block with a pair
        if (mario.blockStandingOn.pair !== undefined) {
            currentLocation.pair = mario.blockStandingOn.pair;
        }
        ...
        // Determine new location based on game and level
        const isSmb1_1 = game === "smb" && level === "1-1";
        const isSmbtllD_1 = game === "smbtll" && level === "D-1";
        ...
        const newX = currentLocation.pair == null ? shiftAmount : currentLocation.pair.constantX - 120;

        // Reset blocks and adjust positions
        ...
        for (let j = 0; j < area.length; j++) {
            ...
            for (let i = 0; i < row.length; i++) {
                ...
                // Adjust firebars if any
                const fireBar = row[i].fireBar;
                if (fireBar) {
                    for (let k = 0; k < fireBar.length; k++) {
                        fireBar[k].drawnX -= newX;
                        fireBar[k].centerX -= newX;
                    }
                }
                ...
                row[i].pair = pair;
                row[i].pulleyPair = pulleyPair;
                if (row[i].pulleyPair != null) row[i].pulleyPair.pulleyPair = row[i];
                row[i].connection = connection;
            }
        }
        ...
        for (let i = 0; i < enemies.length; i++) {
            const enemy = enemies[i];
            if (canResetEnemy) {
                enemies[i] = new Enemy(enemy.constantX, enemy.constantY, enemy.constantWidth, enemy.constantHeight, enemy.storedType, gravity, sounds, currentLocation.terrain, enemy.animated);
            }
            if (enemies[i].drawnX === -1000) {
                canResetEnemy = false;
                enemies.length = i + 1;
        ...
        // Reset Mario state for new location
        mario.keyLastPressed = "right";

        // Set and play new music for location
        music.currentTime = 0;
        music.src = `${pathname}/sounds/${currentLocation.terrain}.wav`;
        music.play();
    };
    ...
    const goBackwards = () => {
        ...
        if (game === "smb" && currentLocation === currentGame.level8_4Castle) {
            ...
            if (timeUntilNextFish <= 0) {
                ...
                const row10 = currentLocation.area[10];
                if (row10[198].drawnX <= 120 && row10[243].drawnX >= 600) {
            ...
            const row13 = currentLocation.area[13];
            const row10_ = currentLocation.area[10];

            if (row13[113].drawnX < 600 && row13[113].drawnX > 590) {
                newX = row13[31].constantX;
            } else if (row13[183].drawnX < 600 && row13[183].drawnX > 590) {
                newX = row13[96].constantX;
            } else if (row10_[254].drawnX < 600 && row10_[254].drawnX > 590) {
                newX = row13[181].constantX;
            }
        }
    ...
    const setDebris = () => {
        for (let i = debris.length - 1; i >= 0; i--) { // iterate backwards to safely splice
            const d = debris[i];

            // Set image source for animation
            const terrain = currentLocation.terrain !== "Bonus" ? currentLocation.terrain : "Underground";
            const frame = (Math.floor(time / 10) % 4) + 1;
            d.img.src = `${pathname}/images/${terrain}BrokenBrick${frame}.png`;
            ...
            if (canUpdate) {
                ...
                d.x += d.velX;
                d.time++;
                d.y = d.originalY - 2 * d.time + 0.5 * gravity * d.time * d.time;

                // Remove if out of screen
                if (d.y > height) {
                    debris.splice(i, 1);
                    continue; // skip drawing removed debris
                }
            }

            // Draw debris
            graphics.drawImage(d.img, Math.round(d.x), Math.round(d.y), 20, 20);
        }
    };
    ...
    const setFireworks = () => {
        ...
        const frame = (Math.floor(fw.existence / 10) % 3) + 1;
        fw.img.src = `${pathname}/images/Firework${frame}.png`;

        fw.existence++;

        // Draw firework
        graphics.drawImage(fw.img, fw.x, fw.y, standardWidth, standardHeight);

        // Remove firework and award points
        if (fw.existence > 30) {
            mario.addScore(500, -100, -100);
            fireworks.shift();
        }
    ...
    function drawHUD() {
        ...
        const coinImageNumber = Math.floor(time % 50 / 10) + 1;
        const coinFrame = coinImageNumber > 3 ? (coinImageNumber === 4 ? 2 : 1) : coinImageNumber;

        if (state !== "loading") {
            currentGame.hudCoin.src = `${pathname}/images/hudCoin${coinFrame}.png`;
        } else if (!currentGame.hudCoin.src.includes(`${pathname}/images/hudCoin1.png`)) {
            currentGame.hudCoin.src = `${pathname}/images/hudCoin1.png`;
        }
    ...
    function handleLoadingState() {
        ...
        if (mario.lives > 0) {
            const smallMario = new Image();
            smallMario.src = `${pathname}/images/smallMarioStanding.png`;
            const centerX = width / 2 - shiftWidth;

            graphics.fillText(`world ${level}`, centerX, 200);
            graphics.drawImage(smallMario, centerX - 75, 240, standardWidth, standardHeight);
            graphics.drawImage(currentGame.xImage, centerX, 250, 20, 20);
            graphics.fillText(mario.lives, 460 - shiftWidth, 270);
        } else {
            graphics.fillText("game over", width / 2 - shiftWidth, height / 2);
        }
    ...
    // Extracted main game state
    function handleGameState() {
        canvas.style.background = currentLocation.color || "black";

        // Reset collision flags
        isHittingLeft = false;
        isHittingRight = false;

        if (mario.blockMovingOn) {
            mario.velX = mario.blockMovingOn.velX;
            mario.blockMovingOn = null;
        }
        ...
        // Determine scrolling
        movingScreen = currentLocation.canScroll && mario.canMoveRight &&
            mario.canMoveLeft && !mario.goingUpPipe && mario.velX > 0;

        updateScrolling();
        isStanding = false;
        setBackground();

        // Draw mario if in pipe transition
        if (mario.transition === "down pipe" || mario.transition === "into pipe" || mario.goingUpPipe) {
            mario.draw();
        }
        ...
        // Handle mario falling
        handleMarioGravity();

        // Draw air/water/lava blocks
        drawBackgroundBlocks();

        // Process enemies
        processEnemies();

        // Draw solid blocks (two passes for layering)
        drawSolidBlocks();
        drawMovingBlocks();
        ...
        // Handle transitions and updates
        handleTransitions();
        updateGameLogic();

        // Draw mario (unless behind castle or in certain states)
        drawMarioFinal();
    ...
    // Block processing (optimized with early exits)
    function processBlocks() {
        const isScrolling = movingScreen && currentLocation.canScroll && mario.drawnX >= width / 2 - shiftWidth;
        ...
        for (let j = 0; j < currentLocation.area.length; j++) {
            ...
            for (let i = 0; i < row.length; i++) {
                ...
                block.update(movingScreen, isScrolling, mario.velX, mario.drawnX + mario.width / 2);

                // Handle bullets
                if (block.bullets?.length > 0) {
                    for (const bullet of block.bullets) {
                        if (!currentLocation.enemies.includes(bullet)) {
                ...
                if (block.type.includes("Coin")) {
                    block.collides(mario);
                } else if (block.type.includes("Flagpole")) {
                    handleFlagpole(block);
                } else if (block.type === "Castle" || block.type === "BigCastle") {
                    handleCastle(block, i);
                } else if (block.type === "Axe") {
                    block.collides(mario);
                } else {
                    handleBlockCollisions(block, row, i, j);
                }
            }
        }
    }
    ...
    function handleBlockCollisions(block, row, i, j) {
        if (mario.goingUpPipe) return;

        const prevRow = currentLocation.area[j - 1];
        const nextRow = currentLocation.area[j + 1];
        let cantShoot = false;

        // Top collisions
        if (prevRow && !prevRow[i].hasCollisions && block.topCollisions(mario)) {
            isStanding = true;
            if (block.type.includes("Cannon")) cantShoot = true;
        }

        // Bottom collisions
        if (nextRow && (!nextRow[i].hasCollisions || nextRow[i].type === "Air") &&
            block.bottomCollisions(mario, addPowerup, addEnemy)) {

            if (block.bumping > 0 && prevRow?.[i].type === "Coin") {
                prevRow[i].type = "Air";
                mario.addCoin(block.drawnX + 10, block.drawnY - standardHeight);
                mario.coins++;
            }
            ...
            if (block.type === "Air") {
                addDebris(createDebrisArray(block));
            }
        }

        // Vine collisions
        if (block.vineStructure || block.type.includes("Vine")) {
            block.vineCollisions(mario);
        }

        // Horizontal collisions
        if (!mario.clearedLevel) {
            if (i > 0 && (!row[i - 1].hasCollisions || row[i - 1].type === "Air") &&
                block.leftCollisions(mario)) {
                isHittingLeft = true;
                if (block.type.includes("Cannon")) cantShoot = true;
            }

            if (i < row.length - 1 && (!row[i + 1].hasCollisions || row[i + 1].type === "Air") &&
                block.rightCollisions(mario)) {
                isHittingRight = true;
                if (block.type.includes("Cannon")) cantShoot = true;
            }
        }
        ...
        if (cantShoot) {
            currentLocation.area.forEach(column => {
                column[i].timeToShootBullet = 100;
            });
        }

        // Enemy collisions (optimized)
        handleBlockEnemyCollisions(block, row, i, j);

        // Powerup collisions
        handleBlockPowerupCollisions(block, row, i, j, nextRow);

        // Fireball collisions
        handleBlockFireballCollisions(block, row, i, j, nextRow, prevRow);
    }

    // Helper functions for clarity
    function createDebrisArray(block) {
        const debris = [];
        const positions = [
            { x: 0, y: 0, velX: -2 },
            { x: block.width, y: 0, velX: 2 },
            { x: 0, y: block.height, velX: -2 },
            { x: block.width, y: block.height, velX: 2 }
        ];

        for (const pos of positions) {
            debris.push({
                img: new Image(),
                x: block.drawnX + pos.x,
                y: block.drawnY + pos.y,
                originalY: block.drawnY + pos.y,
                time: 0,
                velX: pos.velX
            });
        }
    ...
    function handleBlockEnemyCollisions(block, row, i, j) {
        if (!currentLocation.enemies) return;

        const prevRow = currentLocation.area[j - 1];

        for (const enemy of currentLocation.enemies) {
            enemy.isStanding = false;

            if (!enemy.collisions || !enemy.affectedByGravity) continue;

            if (prevRow && (!prevRow[i].hasCollisions || prevRow[i].type === "Air") &&
                block.topCollisions(enemy, mario)) {
                enemy.isStanding = true;
            }

            if (i > 0 && !row[i - 1].hasCollisions && block.leftCollisions(enemy)) {
                enemy.directionFacing = "left";
            } else if (i < row.length - 1 && !row[i + 1].hasCollisions && block.rightCollisions(enemy)) {
                enemy.directionFacing = "right";
            }
        }
    }

    function handleBlockPowerupCollisions(block, row, i, j, nextRow) {
        for (const powerup of powerups) {
            if ((i > 0 && !row[i - 1].hasCollisions && block.leftCollisions(powerup)) ||
                (i < row.length - 1 && !row[i + 1].hasCollisions && block.rightCollisions(powerup))) {
                powerup.velX *= -1;
            }

            if (nextRow && !nextRow[i].hasCollisions && block.bottomCollisions(powerup)) {
                powerup.hitBlock = true;
            }

            const prevRow = currentLocation.area[j - 1];
            if (prevRow && !prevRow[i].hasCollisions && powerup.risen && !block.topCollisions(powerup)) {
                powerup.condition = true;
            }
        }
    }

    function handleBlockFireballCollisions(block, row, i, j, nextRow, prevRow) {
        for (let k = fireballs.length - 1; k >= 0; k--) {
            const fireball = fireballs[k];

            if ((i > 0 && !row[i - 1].hasCollisions && block.leftCollisions(fireball)) ||
                (i < row.length - 1 && !row[i + 1].hasCollisions && block.rightCollisions(fireball)) ||
                (nextRow && !nextRow[i].hasCollisions && block.bottomCollisions(fireball))) {
                sounds[1].currentTime = 0;
                sounds[1].play();
                fireballs.splice(k, 1);
                continue;
            ...
            if (prevRow && !prevRow[i].hasCollisions) {
                block.topCollisions(fireball);
            }
        }
    }

    // Optimized enemy processing
    function processEnemies() {
        if (!currentLocation.enemies) return;

        const isScrolling = currentLocation.canScroll && mario.drawnX >= width / 2 - shiftWidth;

        for (let i = currentLocation.enemies.length - 1; i >= 0; i--) {
            const enemy = currentLocation.enemies[i];

            if (canUpdate) {
                enemy.update(movingScreen, isScrolling, mario.velX, mario.drawnX,
                    mario.drawnY + mario.height / 2, world);

                for (const hammer of enemy.hammers) {
                    hammer.update(movingScreen, isScrolling, mario.velX, mario.drawnX, mario.velX);
            ...
            if (canUpdate && enemy.spinies?.length > 0) {
                for (const spiny of enemy.spinies) {
            ...
            if (enemy.alive) {
                ...
                for (const enemy2 of currentLocation.enemies) {
                    if ((enemy.collisions && enemy2.collisions) || (enemy.moving || enemy2.moving)) {
                        enemy.otherCollisions(enemy2, mario);
                    }
                }
            ...
            handleRedKoopaEdge(enemy);

            // Gravity
            if (canUpdate && !enemy.isStanding && enemy.affectedByGravity &&
                !enemy.isFlying && !enemy.isJumping &&
                !enemy.type.includes("Lakitu") &&
                enemy.type !== "BlooperSwimming" && enemy.type !== "Podobo") {
                enemy.fall();
            }

            // Draw plants
            if (enemy.type.includes("Plant") && enemy.drawnX <= width - shiftWidth &&
                enemy.drawnX + enemy.width >= 0) {
                enemy.draw();
            }
    ...
    function handleRedKoopaEdge(enemy) {
        if (enemy.type !== "RedKoopa" || enemy.inShell || enemy.drawnY !== enemy.lastGroundY) return;

        const rowIndex = Math.floor((enemy.drawnY + enemy.height) / standardHeight);
        if (rowIndex < 0 || rowIndex >= currentLocation.area.length) return;

        const row = currentLocation.area[rowIndex];

        for (const block of row) {
            if (!block.hasCollisions) {
                const dist = enemy.directionFacing === "left" ?
                    enemy.drawnX - block.drawnX : block.drawnX - enemy.drawnX;

                if (dist > 0 && dist < 20) {
                    enemy.directionFacing = enemy.directionFacing === "left" ? "right" : "left";
                    break;
                }
            }
        }
    ...
    function drawBackgroundBlocks() {
        for (const row of currentLocation.area) {
            for (const block of row) {
                if (block.drawnX <= width - shiftWidth && block.drawnX + block.width >= 0 &&
                    ["Water", "Lava", "Air"].includes(block.type)) {
                    block.draw();
                }
            }
        }
    ...
    function drawSolidBlocks() {
        for (const row of currentLocation.area) {
            for (const block of row) {
                if (block.drawnX <= width - shiftWidth && block.drawnX + block.width >= 0 &&
                    !["Water", "Lava", "Air"].includes(block.type) && !block.directionMoving) {
                    block.draw();

                    // Extend ground when level cleared
                    if (mario.clearedLevel && (block.type.includes("Ground") || block.type.includes("Cloud")) &&
                        block.drawnX > newWidth / 2) {
                        const ground = new Image();
                        ground.src = block.img.src;
                        for (let i = 0; i < 10; i++) {
                            graphics.drawImage(ground, block.drawnX + (i * block.width),
                                block.drawnY, block.width, block.height);
                        }
                    }
                }
            }
    ...
    function drawEnemies() {
        for (const enemy of currentLocation.enemies) {
            ...
            if (!enemy.gone && !enemy.type.includes("Plant") &&
                enemy.drawnX <= width - shiftWidth && enemy.drawnX + enemy.width >= 0) {
                enemy.draw();
            }
        }
    }

    // Optimized fireball processing
    function processFireballs() {
        ...
        for (let i = fireballs.length - 1; i >= 0; i--) {
            ...
            if (canUpdate) {
                fireball.update(movingScreen, isScrolling, mario.velX);
            }
            ...
            let shouldRemove = fireball.drawnY > height;

            if (!shouldRemove && currentLocation.enemies) {
                for (const enemy of currentLocation.enemies) {
    ...
    function handleMarioGravity() {
        if (!isStanding && !mario.isJumping && mario.transition === false &&
            !mario.clearedLevel && !mario.goingUpPipe && !mario.onSpring) {
            mario.falling = true;
            mario.isOnGround = false;
    ...
    function updateScrolling() {
        const lastBlock = currentLocation.area[currentLocation.area.length - 1];
        const lastBlockInRow = lastBlock[lastBlock.length - 1];
        ...
        for (const row of currentLocation.area) {
            ...
            if (!possibleToNotScroll) break;
        }

        if (lastBlockInRow.drawnX <= 600 && possibleToNotScroll) {
            currentLocation.canScroll = false;
            handleEndOfLevel();
        } else {
            currentLocation.canScroll = true;
    ...
    function processBowserLogic() {
        if (!currentLocation.enemies) return;

        if (canUpdate) timeUntilNextFlame--;

        const bowsers = currentLocation.enemies.filter(e => e.type === "Bowser");

        for (const bowser of bowsers) {
            if (mario.transition === "cleared castle") {
                bowser.collisions = false;
            }

            // Off-screen bowser flames
            if (world < 8 && timeUntilNextFlame <= 0 && bowser.alive &&
                bowser.drawnX > newWidth && currentLocation.area[0][84].drawnX < newWidth &&
            ...
            // On-screen bowser flames
            if (bowser.alive && bowser.timeToShootFlame <= 0 && bowser.drawnX <= 600) {
                const flameX = bowser.directionFacing === "left" ? bowser.drawnX : bowser.drawnX + bowser.width;
                flames.push(new Projectile("BowserFlame", flameX, bowser.drawnY + 30, bowser.directionFacing, 0, sounds));
                sounds[13].currentTime = 0;
                sounds[13].play();
                bowser.timeToShootFlame = 300;
            }
            ...
            // Hammer throwing
            if ((world >= 8 || world === "D") && bowser.alive) {
                if (canUpdate) bowser.timeToThrow--;

                bowser.throwing = bowser.timeToThrow < 5;

                if (bowser.timeToThrow <= 0) {
                    bowser.hammers.push(new Projectile("Hammer", bowser.drawnX, bowser.drawnY,
                        bowser.directionFacing, 1, bowser.sounds));
    ...
    function processPowerups() {
        ...
        for (const powerup of powerups) {
            if (canUpdate) {
                powerup.update(movingScreen, isScrolling, mario.velX);
            }
            ...
            if (powerup.condition && powerup.type !== "Star" && powerup.risen &&
                !powerup.isJumping && canUpdate) {
                powerup.fall();
    ...
    function processFlames() {
        const isScrolling = currentLocation.canScroll && mario.drawnX >= width / 2 - shiftWidth;

        for (const flame of flames) {
            if (flame.drawnX + flame.width >= 0) {
                if (canUpdate) {
                    flame.update(movingScreen, isScrolling, mario.velX);
                }
    ...
    function processFireBars() {
        const isScrolling = currentLocation.canScroll && mario.drawnX >= width / 2 - shiftWidth;

        for (const row of currentLocation.area) {
            for (const block of row) {
                if (!block.fireBar) continue;

                for (let k = 0; k < block.fireBar.length; k++) {
                    const fire = block.fireBar[k];

                    if (canUpdate) {
                        fire.update(movingScreen, isScrolling, mario.velX);
                    }
    ...
    function updateGameLogic() {
        ...
        canUpdate = [false, "cleared level", "cleared castle", "walkingIntoPipe", "vine"].includes(mario.transition);

        // Check if Mario can stand
        mario.canStand = true;
        for (const row of currentLocation.area) {
            const blockingBlocks = row.filter(block => {
                return block.hasCollisions &&
                    mario.drawnX + mario.width - 5 > block.drawnX + 2 &&
                    mario.drawnX + 5 < block.drawnX + block.width - 2 &&
                    mario.drawnY >= block.drawnY;
            });

            if (blockingBlocks.length > 0 && mario.isCrouching) {
            ...
            if (!mario.canStand) break;
        }
    }

    function drawMarioFinal() {
        if (!mario.behindCastle) {
            if (mario.transition !== "down pipe" && mario.transition !== "into pipe" && !mario.goingUpPipe) {
                mario.draw();
            }
        ...
        if (stage === 4 && mario.transition !== "down pipe" &&
            mario.transition !== "into pipe" && !mario.goingUpPipe) {
            mario.draw();
        }
    }
