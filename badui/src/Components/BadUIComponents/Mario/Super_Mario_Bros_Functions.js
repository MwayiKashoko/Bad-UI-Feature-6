...
export function Player(lives, gravity, music, sounds, coins) {
...
/*Player.prototype = {
	...
	update: function (reset, canScroll, terrain, world) {
		...
		const noTransitionStates = ["walkingIntoPipe", "vine", "climbing vine", "cleared castle"];
		if (this.timeUntilNoTransition <= 0 && isAlive && !this.clearedLevel &&
			!noTransitionStates.includes(transition) && !this.goingUpPipe) {
			this.transition = false;
		}

		// Death check
		if (this.drawnY > height && transition !== "climbing vine" && !this.clearedLevel) {
			this.handleDeath(reset, terrain);
		}

		// Update height based on size
		this.height = (isBig && isAlive) ? this.bigHeight : this.bigHeight / 2;
		...
		if (transition === false && !this.goingUpPipe) {
			this.handleNormalMovement(canScroll, terrain);
		} else {
			this.handleTransitions(transition, world);
		}
		...
		if (!this.hasStar && (this.invincibility % 5 === 1 ||
			(transition === "shrinking" && this.timeUntilNoTransition % 5 === 1))) {
			this.img.src = "";
		}
	},

	handleDeath: function (reset, terrain) {
		if (terrain !== "Sky") {
			if (this.alive) {
				this.timeFromJump = 0;
				this.lastGroundY = height;
				...
				if (musicProgress >= 0.85) {
					...
					if (!this.music.src.includes(`${pathname}/sounds/gameOver.wav`) && this.lives === 0) {
						this.music.src = `${pathname}/sounds/gameOver.wav`;
						this.music.play();
						reset();
					}
	...
	handleNormalMovement: function (canScroll, terrain) {
		const isBig = this.isBig;
		const isOnGround = this.isOnGround;
		...
		const hasZPressed = this.zPressed && terrain !== "Underwater";
		this.isRunning = hasZPressed && (this.leftPressed || this.rightPressed);
		this.isWalking = !this.isRunning && (this.leftPressed || this.rightPressed);

		// Handle crouching
		this.handleCrouching(isBig, isOnGround);

		// Handle horizontal movement
		this.handleHorizontalMovement(terrain);

		// Handle jumping/swimming
		this.handleVerticalMovement(terrain);

		// Handle powerups and states
		this.updatePowerupStates(terrain);
	},

	handleCrouching: function (isBig, isOnGround) {
		const shouldCrouch = ((this.downPressed && isOnGround) || !this.canStand) && isBig;

		if (shouldCrouch) {
			if (!this.isCrouching && isBig) {
				this.drawnY += this.standardHeight / 2;
				this.lastGroundY += this.standardHeight / 2;
			}
			this.isCrouching = true;
			...
			if (this.isCrouching && !isOnGround) {
				// Keep crouching while in air
				this.isRunning = false;
				this.isWalking = false;
				this.width = this.crouchWidth;
				this.height = this.crouchHeight;
				...
				this.isCrouching = false;
				this.height = isBig ? this.bigHeight : this.bigHeight / 2;
				this.width = this.standardWidth;
			}
		}
	},

	handleHorizontalMovement: function (terrain) {
		const isCrouching = this.isCrouching;
		const isBig = this.isBig;

		// Update direction
		if (this.leftPressed || this.rightPressed) {
			this.keyLastPressed = this.leftPressed ? "left" : "right";
			this.directionFacing = this.keyLastPressed;
		}

		// Handle crouching movement in air
		if (isCrouching && isBig && this.isJumping) {
			if (this.leftPressed && this.canMoveLeft && Math.abs(this.velX) < this.maxCrouchingSpeed) {
				this.velX -= this.crouchingFriction;
			} else if (this.rightPressed && this.canMoveRight && Math.abs(this.velX) < this.maxCrouchingSpeed) {
				this.velX += this.crouchingFriction;
			}
		}

		// Calculate acceleration
		if (!isCrouching || !isBig) {
			this.calculateAcceleration(terrain);
		}
	...
	calculateAcceleration: function (terrain) {
		const isUnderwater = terrain === "Underwater";
		const underwaterMultiplier = this.isOnGround ? 0.8 : 1;
		const absVelX = Math.abs(this.velX);

		if (isUnderwater && this.isWalking) {
			const maxSpeed = this.maxSwimmingSpeed * underwaterMultiplier;
			if (absVelX < maxSpeed) {
				this.applyDirectionalFriction(this.standardFriction);
			} else {
				this.velX = Math.sign(this.velX) * maxSpeed;
			}
		} else if (this.isWalking && absVelX < this.maxWalkingSpeed && !isUnderwater) {
			this.applyDirectionalFriction(this.standardFriction);
		} else if (this.isRunning && absVelX < this.maxRunningSpeed && !isUnderwater) {
			this.applyDirectionalFriction(this.standardFriction * 2);
		}
	},

	applyDirectionalFriction: function (friction) {
		if (this.leftPressed && this.canMoveLeft) {
			this.velX -= friction;
		} else if (this.rightPressed && this.canMoveRight) {
			this.velX += friction;
	...
	applyFriction: function () {
		if (this.isChangingDirections !== "none") {
			...
			this.velX *= this.friction + this.standardFriction * 1.2;
		} else if (this.velX !== 0 && !this.isRunning && !this.isWalking) {
			this.velX *= this.friction;
		}

		const threshold = this.isCrouching ? this.standardFriction * 3 : this.standardFriction;
		if (Math.abs(this.velX) < threshold) {
			this.velX = 0;
		}
	},

	handleVerticalMovement: function (terrain) {
		this.lastVelY = this.velY;
		this.swimming = false;

		// Underwater depth limit
		if (terrain === "Underwater" && this.drawnY <= 70) {
			this.drawnY += 2;
		}
		...
		if (this.canPlayJumpSound && this.alive) {
			this.canPlayJumpSound = false;
			const soundIndex = this.isBig ? 10 : 17;
			this.sounds[soundIndex].currentTime = 0;
			this.sounds[soundIndex].play();
		}
	},

	updatePowerupStates: function (terrain) {
		// Star powerup
		if (this.hasStar && this.invincibility > 0) {
			this.invincibility--;
			if (this.invincibility === 0) {
	...
	handleTransitions: function (transition, world) {
		switch (transition) {
			case "cleared level":
				this.handleClearedLevel(world);
				break;
			case "cleared castle":
				this.handleClearedCastle(world);
				break;
			case "growing":
			case "shrinking":
				this.handleSizeTransition();
				break;
			case "dying":
				this.velX = 0;
				this.jump(this.jumpHeight + 1);
			...
			case "into pipe":
				this.handleIntoPipe();
				break;
			case "walkingIntoPipe":
				this.isWalking = true;
				this.drawnX += 1.5;
				break;
			case "vine":
				this.handleVineTransition();
				break;
			case "climbing vine":
				this.handleClimbingVine();
				break;
		}
	...
	handleClearedLevel: function (world) {
		if (this.music.src.indexOf(`${pathname}/sounds/downFlagpole.wav`) === -1 && this.isMovingOnPole) {
			this.music.src = `${pathname}/sounds/downFlagpole.wav`;
			this.music.play();
		}
		...
		if (this.timeUntilGoToCastle >= 5 && this.timeUntilGoToCastle < 50) {
			...
			if (!this.isWalkingToCastle) {
				this.timeFromJump = 0;
				this.lastGroundY = this.isBig ? 440 : 480 - this.height;
				this.drawnX += 15;
			}
			this.isWalkingToCastle = true;
		}
	...
	handleClearedCastle: function (world) {
		this.velX = this.behindCastle ? 0 : 2;
		this.isWalkingToCastle = true;

		if (this.music.src.indexOf("castleClear") === -1 && this.music.src.indexOf("savePrincess") === -1) {
			const musicFile = (world === 8 || world === "D") ? "savePrincess.wav" : "castleClear.wav";
			this.music.src = `${pathname}/sounds/${musicFile}`;
			this.music.play();
		}
	...
	changeLocation: function (transition) {
		...
		if (!["smb", "smbtll"].includes(game)) return;
		...
		switch (transition) {
			...
			case "up pipe":
				this.drawnX = 140;
				this.drawnY = this.isBig ? 440 : 485;
				this.lastGroundY = this.isBig ? 360 : 400;
				this.goingUpPipe = true;
				break;
			case "vine":
				this.drawnX = 160 - this.width / 2 - 3;
	...
	swim: function () {
		this.timeFromJump++;
		this.velY = Math.min(this.timeFromJump - 10, 2);
		this.drawnY += this.velY;
	},
	...
	fall: function () {
		if (!this.canGoToCastle || this.isWalkingToCastle) {
			this.timeFromJump++;
			this.velY = Math.min(this.gravity * this.timeFromJump, 15);
			this.drawnY += this.velY;
		} else {
			this.timeFromJump = 0;
		}
	...
	decideImage: function () {
		...
		if (!this.alive) return "marioDying";

		if (this.transition === "down pipe") {
			return this.isBig ? `${this.type}Crouching` : `${this.type}Standing`;
		}

		if (this.goingUpPipe) return `${this.type}Standing`;

		// Special transitions
		if (["vine", "climbing vine"].includes(this.transition) || this.hasFlippedOnPole) {
			return `${this.type}Clear`;
		}

		// Movement states
		if (this.swimming) {
			type = this.velY < 0 ? "Stroke" : "Swimming";
		} else if (this.isCrouching) {
			type = "Crouching";
		} else if (this.throwingFireball) {
			type = "Shooting";
		} else if (this.isJumping && (!this.isCrouching || !this.isBig)) {
			type = "Jumping";
		} else if (this.isChangingDirections !== "none") {
			type = "Turning";
		} else if (this.isWalking || this.isRunning ||
			(this.transition === "cleared castle" && this.isWalkingToCastle)) {
			type = "Moving";
		}

		// Level clear states
		if (this.clearedLevel) {
			if (this.drawnY + this.height >= 480 || !this.isWalking || this.isJumping) {
				type = "Clear";
			}
		}
	...
	changeImage: function (type) {
		let imgPath = `${pathname}/images/${type}.png`;

		// Animated sprites
		if (type.includes("Moving")) {
			imgPath = `${pathname}/images/${type}${Math.floor(this.time / 10 % 3) + 1}.png`;
		} else if (type.includes("Swimming") || type.includes("Clear")) {
			imgPath = `${pathname}/images/${type}${Math.floor(this.time / 10 % 2) + 1}.png`;
		} else if (type.includes("Stroke")) {
			imgPath = `${pathname}/images/${type}${Math.floor(this.time / 10 % 4) + 1}.png`;
		}

		// Transition animations
		if (this.transition === "shrinking") {
			const isBigFrame = this.time % 20 < 10;
			imgPath = `${pathname}/images/${isBigFrame ? 'big' : 'small'}MarioStanding.png`;
			this.height = isBigFrame ? 80 : 40;
		} else if (this.transition === "growing") {
			if (!this.hasFireFlower) {
				const frame = this.time % 30;
				if (frame < 10) {
					imgPath = `${pathname}/images/smallMarioStanding.png`;
					this.height = 40;
				} else if (frame < 20) {
					imgPath = `${pathname}/images/middleMarioStanding.png`;
					this.height = 60;
				} else {
					imgPath = `${pathname}/images/bigMarioStanding.png`;
					this.height = 80;
				}
			} else {
				this.hasStar = true;
		...
		// Flagpole animation
		if (this.transition !== "cleared castle" && this.clearedLevel && !this.isWalkingToCastle) {
			const frame = Math.floor(this.time / 10 % 2) + 1;
			imgPath = `${pathname}/images/${type}${frame}.png`;

			if (this.timeUntilFallFromFlagpole < 50 || this.canGoToCastle || !this.isMovingOnPole) {
				imgPath = `${pathname}/images/${type}1.png`;
			}
		}
	...
	addCoin: function (x, y) {
		...
		coinAnim.img.src = `${pathname}/images/CoinSpin1.png`;
		this.coinAnimationList.push(coinAnim);
	...
	draw: function () {
		const flipX = Math.round(this.drawnX * 2 + this.width) + shiftWidth;

		if (this.keyLastPressed === "left") {
			graphics.setTransform(-1, 0, 0, 1, flipX, 0);
		}
...
export function Block(type, constantX, constantY, drawnX, drawnY, width, height, sounds, terrain) {
	this.type = type;
	...
	if (this.terrain == "Bonus") {
		this.terrain = "Underground";
	}
...
Block.prototype = {
	...
	draw: function () {
		...
		if (this.drawnY >= 520 && this.terrain != "Underwater" && this.terrain != "Sky" && ["Water", "Air", "Lava"].includes(this.type)) {
...
export function Background(image, x, y, width, height) {
	this.img = new Image();
	this.img.src = image;
	this.drawnX = x;
	this.constantX = x;
...
export function Area(area, terrain, canScroll, color, sounds, background) {
	this.area = area;
	this.terrain = terrain;
...
export function Level(areas) {
	this.areas = areas;
}

export function Enemy(x, y, width, height, type, gravity, sounds, terrain, animated, marioX, bulletX) {
	this.type = type;
	this.storedType = type;
	this.drawnX = x;
...
export function Powerup(x, y, type, gravity, music, sounds, terrain) {
	this.drawnX = x;
	this.drawnY = y;
	this.targetY = y;
	this.width = 40;
...
export function Projectile(type, x, y, direction, gravity, sounds, radius, angle) {
	this.type = type;
	this.canMove = true;
